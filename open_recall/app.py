...
class OpenRecallApp(toga.App):
    ...
    def create_loading_screen(self):
        ...
        # Add loading text
        title_label = toga.Label("Open_Recall", style=Pack(text_align="center", font_size=24, padding=(20, 0)))
        loading_box.add(title_label)

        loading_label = toga.Label(
            "Starting server, please wait...", style=Pack(text_align="center", font_size=16, padding=(10, 0))
        )
    ...
    def create_main_view(self, sender=None):
        ...
        web_view = toga.WebView(
            url=f"http://{APP_HOST}:{self.port}", style=Pack(flex=1)  # This makes it take up all available space
        )

        # Create main box with the WebView
        main_box = toga.Box(
            children=[web_view], style=Pack(direction=COLUMN, flex=1)  # flex=1 makes it fill the window
        )

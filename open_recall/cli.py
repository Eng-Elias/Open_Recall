...
from open_recall.main import load_config
...
def get_parser():
    ...
    parser = argparse.ArgumentParser(description="Open_Recall - Find and analyze anything you've seen on your PC")
    ...
    server_parser.add_argument(
        "--host", default=None, help="Host to bind the server to (default: from config or 127.0.0.1)"
    )
    server_parser.add_argument(
        "--port", type=int, default=None, help="Port to bind the server to (default: from config or 8000)"
    )
    server_parser.add_argument("--no-browser", action="store_true", help="Don't open a browser window automatically")

    # Desktop command
    subparsers.add_parser(CommandEnum.DESKTOP.value, help="Start the Open_Recall desktop application")

    # Version command
    subparsers.add_parser(CommandEnum.VERSION.value, help="Show Open_Recall version")

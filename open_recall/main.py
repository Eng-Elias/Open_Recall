...
from open_recall.utils.db_utils import Base, Screenshot, Tag, engine, get_db
from open_recall.utils.schemas import (
    BaseModel,
    EventType,
    Page,
    ScreenshotResponse,
...
from open_recall.utils.summarization import (
    download_model,
    generate_search_results_summary,
    generate_summary,
...
async def delete_tag(tag_id: int, db=Depends(get_db)):
    ...
    await manager.broadcast({"type": EventType.TAG_DELETED, "tag_id": tag_id})
...
async def update_notes(screenshot_id: int, notes_data: NotesUpdate, db=Depends(get_db)):
    ...
    await manager.broadcast(
        {"type": EventType.NOTES_UPDATED, "screenshot_id": screenshot_id, "notes": screenshot.notes}
    )
...
async def toggle_favorite(screenshot_id: int, db=Depends(get_db)):
    ...
    # Broadcast the update
    await manager.broadcast(
        {"type": EventType.FAVORITE_UPDATED, "screenshot_id": screenshot_id, "is_favorite": screenshot.is_favorite}
    )
...
async def add_tag_to_screenshot(screenshot_id: int, tag_id: int, db=Depends(get_db)):
    ...
    if tag not in screenshot.tags:
        ...
        # Broadcast the update
        tag_response = {"id": tag.id, "name": tag.name}
        await manager.broadcast({"type": EventType.TAG_ADDED, "screenshot_id": screenshot_id, "tag": tag_response})
...
async def remove_tag_from_screenshot(screenshot_id: int, tag_id: int, db=Depends(get_db)):
    ...
    if tag in screenshot.tags:
        ...
        # Broadcast the update
        await manager.broadcast({"type": EventType.TAG_REMOVED, "screenshot_id": screenshot_id, "tag_id": tag_id})
...
async def delete_screenshots_before_date(
    ...
    try:
        ...
        if exclude_favorites:
            query = query.filter(not Screenshot.is_favorite)

        if exclude_with_notes:
            query = query.filter(
                or_(Screenshot.notes is None, Screenshot.notes == "", func.length(Screenshot.notes) == 0)
            )
        ...
        file_paths = [
            os.path.join(screenshot_manager.storage_path, screenshot.file_path)
        ...
        await manager.broadcast({"type": EventType.SCREENSHOTS_DELETED, "count": count, "ids": deleted_ids})

        return {"success": True, "message": message, "count": count, "files_deleted": deleted_files}
...
async def update_settings(settings: SettingsUpdate):
    ...
    await manager.broadcast({"type": EventType.SETTINGS_UPDATED, "settings": updated_settings})
